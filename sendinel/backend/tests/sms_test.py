...
class SMSTest(unittest.TestCase):
    ...
    def test_send_sms_commands_pdu_and_correct_len(self):
        ...
        """
        recipient = '012345'
        message = 'this is the message\n via our serial connection\n we just want to test the behaviour\n becuase often it works wrong, but our app is pretty cool, but for correctness we test all methods'
        pdu = PDU()
        pdu_string =  pdu.encodeSMS(recipient,message[0:160])
        
        
        self.counter = 0
        self.commands = ['AT\r','AT+CMGF=0\r','AT+CSMS=0\r','AT+CMGS=%i\r'%((len(pdu_string)-2)/2),pdu_string ,chr(26)]        
        def write(command):
            self.ser.write1(command)
            self.assertEquals(self.commands[self.counter], self.ser.read(len(self.commands[self.counter])))
            self.counter += 1
