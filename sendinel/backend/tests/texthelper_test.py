...
class TesthelperTest(unittest.TestCase):
    ...
    def test_generate_text_message(self):
        text = texthelper.generate_text({'free_text':"Mrs. Joirie, your " + \
                        "medicine is there. " + \
                        "Please remember to pay 2 $."}, Template("$free_text"))
        should_text = texthelper.SMS_SALUTATION + "Mrs. Joirie, your " + \
                        "medicine is there. Please remember to pay 2 $."
        self.assertEquals (text, should_text)
        self.assertTrue(len(text) <= 160)
    ...
    def test_generate_notification_text_cut_off(self):
        ...
        should_text = texthelper.SMS_SALUTATION + "What an interesting text" + \
    ...
    def test_generate_notification_text_no_cut(self):
        ...
        should_text = "What an interesting text :Hello my name is james I " + \
                            "am 22 years old and i really want to go to " + \
                            "bali and make holidays since my last holidays " + \
                            "are ages ago and i think a " + \
                            "little bit less stress would be fun to have"
